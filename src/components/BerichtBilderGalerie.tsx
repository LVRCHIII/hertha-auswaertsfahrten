  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold uppercase tracking-widest text-shell-fg/45">
        ...
        {canEdit ? (
          <button
            ...
            className="flex items-center gap-1 rounded-lg bg-shell-fg/10 px-3 py-1.5 text-xs font-semibold text-shell-fg/80 hover:bg-shell-fg/15 disabled:opacity-50 transition"
          >
      ...
      {bilder.length === 0 && !canEdit ? null : bilder.length === 0 ? (
        <button
          ...
          disabled={busy}
          onClick={() => fileInputRef.current?.click()}
          className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-dashed border-shell-fg/15 py-8 text-sm text-shell-fg/45 hover:border-shell-cta-bg hover:text-shell-fg transition disabled:opacity-50"
        >
        ...
        <div className="grid grid-cols-3 gap-2">
          {bilder.map((bild) => (
            <div
              key={bild.id}
              className="group relative aspect-square overflow-hidden rounded-xl bg-shell-fg/10"
            >
              <img
                src={bild.url}
                alt=""
          ...
          {canEdit ? (
            <button
              ...
              disabled={busy}
              onClick={() => fileInputRef.current?.click()}
              className="flex aspect-square items-center justify-center rounded-xl border-2 border-dashed border-shell-fg/15 text-2xl text-shell-fg/30 hover:border-shell-cta-bg hover:text-shell-fg transition disabled:opacity-50"
